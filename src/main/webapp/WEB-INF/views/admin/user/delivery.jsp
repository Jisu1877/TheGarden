...
<head>
    ...
    <script>
    	function defaultChange(idx) {
    		const user_idx = ${userVO.user_idx};
    		const user_id = '${userVO.user_id}';
			$.ajax({
				type : "post",
				url : "${ctp}/admin/user/delivery/defaultChange",
				data : {user_idx : user_idx, user_delivery_idx : idx},
				success : function() {
					alert("기본 배송지 변경이 완료되었습니다.");
					location.href = '${ctp}/admin/user/userInforUpdate?user_id=' + user_id + '&target=delivery-tab';
				},
				error : function() {
					alert("전송오류.");
				}
		...
		<div class="w3-row-padding w3-padding-16">
			...
			<div class="w3-col m10 w3-margin-bottom">
				...
				<c:if test="${deliveryVO.default_flag == 'n'}">
					...
					<a href="${ctp}/admin/user/delivery/delete?user_delivery_idx=${deliveryVO.user_delivery_idx}&user_id=${userVO.user_id}" class="w3-right">[삭제]</a>
				</c:if>
